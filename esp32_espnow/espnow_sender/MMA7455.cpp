...
int MMA7455_init(void)
{
  ...
  if (error != 0) {
    Serial.println("fail to write to MMA7455_MCTL");
    return (error);
  }
  ...
  if (error != 0) {
    Serial.println("fail to read from MMA7455_MCTL");
    return (error);
  }
  ...
  if (error != 0) {
    Serial.println("fail to write to MMA7455_XOFFL");
    return (error);
  }
...
#define USE_INTERNAL_OFFSET_REGISTERS
  ...
  if (error != 0) {
    Serial.println("fail to read MMA7455_xyz");
    return (error);
  }
